...
const Conversation = (props) => {
  ...
  const [existingChatLogs, setExistingChatLogs] = useState([]);
  ...
  const createSocket = () => {
    ...
    let cable = Cable.createConsumer(
      `${path}/api/v1/cable/conversation/${props.location.state.id}?token=${token}&uid=${uid}&client=${client}&locale=${lang}`
    );
    chats = cable.subscriptions.create(
      {
        channel: 'ConversationsChannel',
        conversations_id: props.location.state.id,
      },
      {
        connected: () => {},
        received: (data) => {
          let chatLogs = existingChatLogs;
          chatLogs.push(data.message);
          setExistingChatLogs(chatLogs);
  ...
  useEffect(() => {
    ...
    if (window.navigator.onLine === false) {
      ...
      const path = `/api/v1/conversations/${props.location.state.id}?locale=${lang}`;
      axios
  ...
  const handleSendEvent = (event) => {
    ...
    if (window.navigator.onLine === false) {
      ...
      axios
        ...
        .then(() => {
          if (uploadedImage !== '') {
            chats.create(newMessage, uploadedImage, props.location.state.id, props.id);
            setNewMessage('');
  ...
  const deleteConversation = () => {
    ...
    if (window.navigator.onLine === false) {
      ...
      if (window.confirm(t('SingleConversation:del-conversation'))) {
        const path = `/api/v1/conversations/${props.location.state.id}`;
        const headers = {
          uid: window.localStorage.getItem('uid'),
          client: window.localStorage.getItem('client'),
          'access-token': window.localStorage.getItem('access-token'),
      ...
      <>
          ...
          <div className='max-width-wrapper' style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <div
              style={{
                display: 'flex',
                margin: 'auto',
                cursor: props.location.state.user.id !== null && 'pointer',
              }}
              onClick={() => {
                props.location.state.user.id !== null &&
                  props.history.push({
                    pathname: '/host-profile',
                    state: {
                      userId: props.location.state.user.id,
                      avatar: props.location.state.user.profile_avatar,
                      nickname: props.location.state.user.nickname,
                      location: props.location.state.user.location,
                      errors: '',
                      noMessage: true,
                    },
                  });
            ...
            >
              <Image
                src={
                  props.location.state.user.profile_avatar === null
                    ? `https://ui-avatars.com/api/?name=${
                        props.location.state.user.nickname === 'Deleted user'
                          ? '[x]'
                          : props.location.state.user.nickname
                      }&size=150&length=3&font-size=0.3&rounded=true&background=d8d8d8&color=c90c61&uppercase=false`
                    : props.location.state.user.profile_avatar
                }
                size='mini'
        ...
        >
          <div className='single-conversation-wrapper' style={{ paddingTop: '80px' }}>
            ...
            {messagesHistory.length < 1 && existingChatLogs.length < 1 && errorDisplay === false && (
                ...
                return (
                  <div key={message.id}>
                    <MessageBubble
                      ...
                      otherAvatar={props.location.state.user.profile_avatar}
                      message={message}
            ...
            {existingChatLogs.length > 0 &&
              existingChatLogs.map((message) => {
                return (
                  <div key={message.id}>
                    <MessageBubble
                      currentUsername={props.username}
          ...
          <div className='single-conversation-wrapper'>
            <div style={{ display: 'inline-flex', width: '100%' }}>
              <Icon
                ...
                size='big'
                style={{
                  display: (props.location.state.user.id === null || newMessage.length > 0) && 'none',
                  cursor: 'pointer',
                  color: '#d8d8d8',
                  fontSize: '2.5em',
                  marginRight: '0.5rem',
