    ...
    if (props.requests.length > 0) {
      return (
        <>
          ...
          <p style={{ textAlign: 'center' }}>{t('OutgoingRequests:desc')}</p>
              ...
              >
                <p className='small-centered-paragraph'>
                  <Trans count={parseInt(request.number_of_cats)} i18nKey='OutgoingRequests:req-desc'>
                    You have requested to book a stay with <strong>{{ nickname: request.host_nickname }}</strong> for
                    your <strong>{{ count: request.number_of_cats }} cat</strong> during the dates of
