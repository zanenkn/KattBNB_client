        ...
        {sortedRequests.map(request => {
          return (
            <Container style={{ 'backgroundColor': '#e8e8e8', 'marginBottom': '3rem', 'padding': '2rem' }} id={request.id} data-cy='incoming-requests' key={request.id}>
              <p className='small-centered-paragraph'>
                <strong>{request.host_nickname}</strong> wants to book a stay for their <strong>{request.number_of_cats} {request.number_of_cats > 1 ? 'cats' : 'cat'}</strong> during the dates of <strong>{moment(request.dates[0]).format('YYYY-MM-DD')}</strong> until <strong>{moment(request.dates[request.dates.length - 1]).format('YYYY-MM-DD')}</strong>.
              </p>
              <p className='small-centered-paragraph'>
