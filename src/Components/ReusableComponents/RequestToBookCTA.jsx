      ...
      <>
        <p className='small-centered-paragraph' style={{ marginBottom: '0.5rem' }}>
          <Trans i18nKey='reusable:request-cta.txt' count={parseInt(props.numberOfCats)}>
            The stay for <strong style={{ color: '#c90c61' }}>{{ count: props.numberOfCats }} cat</strong> with
            <strong style={{ color: '#c90c61' }}>{{ host: props.nickname }}</strong> during the dates of
            <strong style={{ color: '#c90c61' }}>{{ checkin: moment(props.checkInDate).format('YYYY-MM-DD') }}</strong>
            until
            <strong style={{ color: '#c90c61' }}>
              {{ checkout: moment(props.checkOutDate).format('YYYY-MM-DD') }}
            </strong>
            would in total cost
          </Trans>
        </p>
