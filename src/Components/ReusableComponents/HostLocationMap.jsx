...
class HostLocationMap extends Component {
  ...
  render() {
    ...
    if (this.props.tReady) {
      ...
      googleMapsLink = `https://www.google.com/maps/search/?api=1&query=${parseFloat(this.props.lat)},${parseFloat(
        this.props.long
      )}`;
        ...
        underMapText = (
          <p className='small-centered-paragraph' style={{ marginBottom: '0.5rem' }}>
            <Trans i18nKey='HostLocationMap:under-map-txt' values={{ nickname: this.props.nickname }}>
              This is the approximate area of <strong style={{ color: '#c90c61' }}>{this.props.nickname}</strong>. You
              will receive the exact location when booking is confirmed.
            </Trans>
          </p>
        );
      } else {
